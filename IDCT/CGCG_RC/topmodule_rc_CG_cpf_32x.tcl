...
set _OUTPUTS_PATH outputs_My_Files_CGcpf_32X
set _REPORTS_PATH reports_My_Files_CGcpf_32X 
...
#Switching Info
####################################################################
#read_tcf -instance [find / -inst XLXI_3] -verbose /home/muslim/Desktop/prof_method/CGCG_RC/constraints/mmmc/top_CGnew_32X.tcf
...
#########write_hdl -lec > ${_OUTPUTS_PATH}/${DESIGN}_intermediate_CGcpf_32X.v
###############Revised CPF for LEC##################
#write_cpf -output_directory ${_OUTPUTS_PATH} -prefix intermediate 

#########report timing >  $_REPORTS_PATH/${DESIGN}_timing_CGcpf_inter_32X.txt
#########report gates >   $_REPORTS_PATH/${DESIGN}_gates_CGcpf_inter_32X.txt
#########report area >    $_REPORTS_PATH/${DESIGN}_area_CGcpf_inter_32X.txt
...
read_tcf -verbose /home/muslim/Desktop/prof_method/CGCG_RC/constraints/mmmc/my4_32X.tcf

##write_encounter design -basename <path & base filename> -lef <lef_file(s)>
verify_power_structure -post_synthesis -license lpgxl> $_REPORTS_PATH/${DESIGN}_CGcpf_verify_power_struct_32X.rpt
#suspend

###########Revised design after incremental opt for LEC#####################
write_hdl -lec > ${_OUTPUTS_PATH}/${DESIGN}_incremental_CGcpf_32X.v
###############Revised CPF after incremental opt for LEC####################
#write_cpf -output_directory ${_OUTPUTS_PATH} -prefix incremental

report timing > $_REPORTS_PATH/${DESIGN}_timing_CGcpf_incr_32X.txt
report gates >  $_REPORTS_PATH/${DESIGN}_gates_CGcpf_incr_32X.txt
report area >   $_REPORTS_PATH/${DESIGN}_area_CGcpf_incr_32X.txt
puts "Total area [get_attr area /designs/topmodule_rtl]"
timestat FINAL
puts "============================"
puts "Synthesis Finished ........."
puts "============================"
report power > ./Power_Reports/pwrrpt_CGcpf_32X.txt
report power -mode PM1 > ./Power_Reports/pwrrpt_CGcpf_32X_PM1.txt
report power -mode PM2 > ./Power_Reports/pwrrpt_CGcpf_32X_PM2.txt
write_encounter design -basename TEST1_CGcpf_32X/test2_CGcpf_32X
...
#write_tcf -computed > /home/muslim/Desktop/prof_method/CGCG_RC/constraints/mmmc/my4_32X.tcf
